...
namespace WMS_client.Processes.Lamps
    ...
    public class PlacingOnMap : BusinessProcess
        ...
        public PlacingOnMap(int map, Int16 register, byte position)
            : base(1)
            {
        ...
        public override void OnBarcode(string barcode)
            ...
            if (barcode.IsAccessoryBarcode())
                ...
                if (_Case == null)
                    {
                    "Не знайдено корпусу з таким штрих-кодом!".ShowMessage();
                    return;
                    }

                if (_Case.Lamp == 0)
                    {
                    "Лампа не вказана! Операція відмінена!".ShowMessage();
                    return;
                    }

                if (_Case.Unit == 0)
                    {
                    "Електронний блок не вказаний! Операція відмінена!".ShowMessage();
                    return;
                    }
                ...
                if (!Configuration.Current.Repository.UpdateCases(new List<Case> { _Case }, false))
                    {
                    "Не вдалося оновити світильник!".ShowMessage();
                    return;
                    }
